...
api_key = "tTeftsYmE1X6ReGuuluHzasgiQqvSpWpiQlCQvqCmpI4qRkVXb3c2aMSIdjA0I7z"

api_secret = "GuWGrEsqwKLOVqsLy93UGyiCxeeav53xqge4SeJTieQdb8slkC3VLXAS56M2DKla"
...
async def run_websocket():
    ...
    client = Client(api_key, api_secret)
    trade_coin = input("What coin will you trade?")
    coin_price = client.get_symbol_ticker(symbol=trade_coin)
    trade = float(input(f"{trade_coin} price is {coin_price} what is the price you want to buy {trade_coin} ? "))
        ...
        while Verdad == True:
            ...
            def watch_coin():
                ...
                if  float(open_price) <= trade:
                    print(f" BUY BUY BUY your buy price is {trade} the {trade_coin} price is {Fore.green}{open_price}")
                    buy_coin()
                                #rgb(255,140,0) dark orange
                else: print(f" {Fore.rgb(255, 140, 0)}NOT READY your buy price is {Fore.rgb(0, 128, 0)}{trade} {Fore.white}the {trade_coin} price is {Fore.red}{open_price}")
